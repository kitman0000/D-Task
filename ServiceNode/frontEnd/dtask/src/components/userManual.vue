<template>
	<div style="line-height: 30px;font-family: '微软雅黑';">
		<h2 style="font-size: 40px;">Tasking 用户手册</h2>
		<h3>项目说明</h3>
		<p>
			Tasking是一款由DropTable开发的，面向团队的分布式任务管理软件，支持团队管理，行程管理等功能。
			</br>
			普通的团队任务管理软件，通常部署在一台服务器上，整个团队使用一个系统。但这样一旦系统需要更新，迁移，甚至发生崩溃，就会导致整个团队无法使用。
			Tasking将让每个部门拥有自己独立的任务管理服务，每个服务称之为节点。节点与节点之间可以互相连接，（例如一个公司有北京总部，上海分部，广州分部，
			则上海分部和广州分部可以连接至北京总部），组建一个强大的任务管理网络。
		</p>
		...
		<p>
			...
			Tasking部分数据(例如验证用户身份的token)通过内存储存，从而增加读写速度，我们允许用户修改缓存设置，但是这个选项只有在重启后才生效。</br>
		...
		<h3>任务系统</h3>
		<p>
			任务系统是Tasking的核心业务，这里储存了所有的任务。任务系统分为本地任务模块和联合任务模块，本地任务模块为当前服务节点内部的任务管理，联合任务模块是整个Tasking网络上共同的
			任务管理，其他功能完全相同。关于节点和网络，请参见<strong>“服务节点”</strong>和<strong>“Tasking网络”</strong>。</br>
			任务系统中，用户可以参与任务。
		...
		<h3>服务节点</h3>
		<p>
			服务节点是指一个拥有界面的Tasking服务，由前端和后端组成。它通过RabbitMQ消息队列与其他服务节点和中心调配服务通讯。每个团队的分支（例如：上海部分，杭州部分）通过访问
		...
		<h3>Tasking网络</h3>
		<p>
			由服务节点和中心调配服务共同搭建的网络称之为Tasking网络，一个Tasking网络中，可以有多个服务节点，但只有一个中心调配服务。将它们部署在一个消息队列中，就可以使用。具体网路构造
		...
		<h3>代码开源</h3>
		<p>
			Tasking源码完全开放，欢迎有志之士参与开发。<a href="https://github.com/kitman0000/D-Task"  target="view_window">点击查看</a>
		</p>
		</br>
		</br>
